...
public abstract class GenTemplate<PrimaryType> extends AbstractGenTemplate {
	 ...
	 * <p>
	 * If the extending class overrides #getPrimarySource, we defer to that (at this
	 * point our own implementation of #getPrimarySource is guaranteed to return
	 * null, a non-null value must come from an override). Otherwise, we resolve a
	 * primary source specified through configuration, if any. Either way, we place
	 * the final result in the context for all to see.
	 * 
	...
	/**
	 * @deprecated GenTemplates should extend {@link StaticGenerator} instead, and
	 *             override {@link #getStaticGenerator()}.
	 */
	@Deprecated
