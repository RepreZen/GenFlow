...
import com.reprezen.genflow.rapidml.diagram.XDiagramGenTemplate
import com.reprezen.rapidml.ZenModel
import java.util.Map

import static com.reprezen.genflow.common.HtmlInjections.*

class XGenerateD3JS extends ZenModelOutputItem {

	static val URL_PREFIX_PARAM = 'urlPrefix'
	static val ANCHOR_NAME_PARAM = 'anchorName'
	...
	override generate(ZenModel model) {
		// jsonData is only used for preview, otherwise it is imported from /data/<modelName>.js  
		val jsonData = if (preview) {
				val generator = new XGenerateJSON()
				generator.init(context)
				generator.generateJSON(model)
			}
		generate(jsonData, model.name)
	}
	...
	def String generate(CharSequence data, String modelName) {
		...
		val anchorName = getStringParameter(ANCHOR_NAME_PARAM, null)

		var pathToJson = '''«urlPrefix»/data/«modelName».js''';
		val pathToJsonParamName = GenTemplate::getParamNameFor(XDiagramGenTemplate::JSON_OUTPUT_ITEM_NAME);
