...
public class OpenApiGeneratorGenTemplateGroup implements IGenTemplateGroup {
	...
	public Iterable<IGenTemplate> getGenTemplates(ClassLoader classLoader) {
		// the incoming classloader will cover custom SCG modules in the workspace, but
		// it doesn't support discovery of
		// SCG modules from the standard SCG distribution. I'm not entirely sure of why,
		// and for now the workaround is
		// to merge it with the classloader that loads CodegenConfig, which is able to
		// find everything.
		ClassLoader mergedLoader = new MergedClassLoader(CodegenConfig.class.getClassLoader(), classLoader);
		OpenApiGeneratorModulesInfo modulesInfo = getModulesInfo();
		List<IGenTemplate> genTemplates = Lists.newArrayList();
		for (Class<? extends CodegenConfig> config : getCodegenConfigClasses(modulesInfo, mergedLoader)) {
